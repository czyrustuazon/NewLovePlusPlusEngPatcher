...
The DATA apply chain (states 22+) stays idle, including jump-table slots 24–31 / 41–42 at `0x150658`. After the help line, `0x14EFE8` is `mov r1, #2` so state 2 (`0x14FAC0`, `FUN_0014d584`) draws the two right-hand bars. `apply_patch` upgrades a `code.bin` that still has the old skip or the state-21 handoff. Ships in `release/name_input_code.bin` and the Azahar instance `exefs/code.bin`.
...
| Watcher magazine | `0x006092C8` / `0x00609164` / `0x00609600` / `0x006093F4` / `0x006090B0` | `mov r0,#1; bx lr`. **Not** `FUN_005fe9d8` (has-rows must poll). MagList always command **9**, then **state 6** until rows bind; keep layout `+0x7c`. Parent skips MagList-equal gate and after MagList goes **0x17** (dismiss), not Watcher. NOP MagList pump / has-rows / bind `+0x45/+0x46` and parent state 0 so `FUN_005fe5d0` inits. Watcher extra `bne` restored. WEB **とわのウォッチャー** is Myroom layout **0xC** (`FUN_0005b49c` @ `0x0005B49C`), not TownGuide MagOp layout `0x31`. Extra-data `FUN_004e7d34==0` or article `+0x70==0` set `+0x5e=9` (empty viewer → ERROR “No data.”). NOP the extra `beq` @ `0x0005B570`; `+0x70` miss and empty `FUN_00444cd8` count (`0x0005B600` / `0x0005BC74` / `0x0005AC04`) go hub state **7**. State 9 entry `0x0005BA44` `b 0x5ba20`. Watcher pill buttons 9 and 10 (`0x0005B6DC`, `0x0005B724`) branch to the click epilogue, so neither builds the empty viewer. `0x0005B574` also goes to hub state 7. Empty count @ `0x0005BC70` dismisses the page. `FUN_0005b49c` stays vanilla. Myroom 0x84 ERROR `0x0004FC20`/`0x0004FC58` and leftover pack `0x9d01` slot 8 `0x0014E110` are NOP. MagOp: extra-missing left Watcher `0x31` empty (pack `0xB200`). State 0 goes **5** (no layout), extra cmp `mov r0,#1`, parent after Watcher **0xC** MagList (cart `Mag_Tit`). `FUN_0014eb9c` returns `(extra & ~bit0x20 & has-rows) ^ 1` and binds pack `0x9d01` when extra is **missing** (slot `0x60`) or extra+no bit+no rows (slot `0xf`). Stub the entry `mov r0,#0; bx lr` @ `0x0014EB9C`. Caller leftover `+0xb4 != -1` still opened the 0x84 ERROR window; `0x0014E16C` `beq` → `b 0x14e2a4`. Inner `bl FUN_005fe9d8` stays vanilla. |
...
### 16.9 In-room Watcher ERROR card is the Town Guide book (2026-09-24)

The bottom-screen **ERROR / sentence / OK** box on the in-room とわのウォッチャー tap is **not** `FUN_0005b49c`, message id 80, the layout-29 store at `0x000F3ED8`, or the menu mapper at `0x00100DA0`. Those were patched in turn and the card stayed. §16.8’s “Myroom layout 0xC, not TownGuide” line is that other operator. This tap is the tick at **`0x003E67C8`** (vtable word at file `0x00722EB0`).

`FUN_00195fa4` both links a one-line message record and returns a packed layout id. The sentence is pack **`0x1901`**, slot = table A[`layout`] + `mode` (halfwords at file `0x006C6F5C`). Slot **356** is STRI **3346** `データがありません。` Table A base **355** is layout **29**, so mode **1** is the only pair that prints that sentence. State 6 (`0x003E6DB8`) does exactly that when `[this+0x88]==0`:

```
ldrb r0, [r4, #0x88]
cmp  r0, #0
moveq r1, #0x1d    @ 0x003E6DC0 layout 29
moveq r2, #1       @ mode 1
beq  dialog        @ 0x003E6DC8 → bl FUN_00195fa4
```

Rewriting `r1` to layout `0x31` inside that call changed the sentence (SpotPass opt-in) and left the ERROR skin. The function cannot draw magazine rows. Returning 0 from it bounces to the selection. Returning a packed id with no record blanks the bottom screen. State 6’s OK tail (`0x003E6E80`, state `0xB`) is the same menu loop.

The book is state 3 (`0x003E6908`). A blob whose first byte is 0 used to `moveq r0,#6; beq 0x3e6d90` and come back as this card. The success tail at **`0x003E6D28`** registers that buffer and `bl 0x4dcf30`, which constructs **`TownGuideUIOperator`** and links it on `this+0x60`, then state 4. `src/patch_spotpass_embed.py` retargets the empty `beq` at **`0x003E6948`** from `0x3e6d90` to **`0x3e6d28`**, and retargets state 6’s empty `beq` at **`0x003E6DC8`** to state 3 so the card is never built. Setting state 4 without that `bl` is the blank screen (the selection is already gone and no window was linked).
...
This restores **access to the magazine viewer**. The cart spread that used to open here is issue 2 (pages 01/02, New Open). **§16.11** replaces that with issue 28’s holy-site corner: the lake paragraph on the right page, 「湖」 on the SPOT row, and 奥十羽野駅 in the AREA text slot. `FUN_006088c8` stays a stub. The AREA name is a cave draw into `Tex_Place`, not a merge fill.
...
Issue 28’s cleartext body is six city comments, one lake paragraph, and one heated-pool paragraph. The city comments belong on the map (街の声), not as extra magazine pages. The lake sentence is the pink 「恋愛の聖地」NEW誕生 corner (`Mag_Tit03`). The pool sentence is FEVER 今週の人気デートスポット (`Mag_Tit05`). Both corners use `Mag_Page01`, so only the lake corner is shown. A weekly file supplied the place strings, and the game drew them into one-sided text panes (`Tex_Name_00` / `Tex_Place_00`, 120×12, under `Anm_Rot` on `Lyt_Spot_O01`). Merge never fills those buffers. The AREA name below is a cave draw into `Tex_Place`.
...
SPOT is a short place name and AREA is the station (issues 1–3: 海水浴場 / 臨海駅, then 噴水公園 / 新とわの駅). Issue 28’s lake record (`02 02 01 91` then the paragraph at `info.dat` offset 1415) names only 「湖」. The station is the game’s own 奥十羽野駅: the map area 奥十羽野, and the Love Plus grouping that puts 十羽野湖 under 奥十羽野駅周辺. `info.dat` has no station field on this record.

「湖」 stays on the badge. The layout’s texture list names the green set (`Mag_Obj_Spot01`). After load, the page swaps in a color set from the name table at file `0x00738CD8` (five groups: Area / Line / Mark / Spot, plus the matching Deco). The holy-site corner uses set 02, the pink badge `Mag_Obj_Spot02`. Painting only `Spot01` never appeared. The deploy scales each `Mag_Obj_Spot01`–`05` (72×32 RGBA4444) into the left 48 pixels and draws 「湖」 in the right 24. `Pic_Spot` in `Lyt_Spot_O01` and `Pts_Spot_O01` is widened from 72×32 at x=−60 to 108×32 at x=−42, so the left edge stays at −96 and the character sits on the row. The underline pane is unchanged. Putting a name on `Pic_Line01` did not work: the game rebinds the line to `Mag_Spot_Line02` (16×16, stretched). Painting 奥十羽野駅 into the AREA badge’s unused 128-wide pixel buffer showed the five characters, and they were soft. The AREA badge stays the vanilla 72×32 graphic.

The station name is the game font in `Tex_Place` (pane index 1, 120×12, on the pink AREA rule). The name refresh at `0x004DCA94` draws when `[magazine+0x5c]!=0`. Page index 0 uses the inline record at `this+0x90`. SPOT would be the bytes at `record+4`; AREA would be the bytes at `record+0x84`. Merge never writes them. `0x004DCBC0` calls `0x002469D8` (index 0, `Tex_Name`) and `0x004DCBCC` calls `0x002469CC` (index 1, `Tex_Place`). Both tail-call `0x00199C20`, which MakeStrs `r2` and DrawTextToPanes the pointer at `widget+0x74+index*4`. A null pane returns without drawing.

The binder at `0x00246838` is what stores those pane pointers. It treats `[widget+4]` as a boolean and searches layout group 0 when that pointer is null, so with merge stubbed the lookup misses and `Tex_Place` stays null. Merge stays the stub. The binder stays uncalled: its first call, `0x00225B58`, rebinds the whole widget. The hook replaces the three instructions at `0x004DCBCC` with a branch to a cave at `0x0014EBC8`, in the dead body of the ret0 gate, after the vanilla has-rows BL at `0x0014EBC4` and before that function’s `pop` at `0x0014ECF8`. `r8` is still the spot widget. If `[r8+0x78]` is null, the cave sets `[r8+0x70]=1` so the store lands on index 1, looks the layout up with key `[r8+8]` in group 1 and then group 0 (`0x005C6E84`), finds `Pos_Spot_O01` (`0x005EBA00`), and stores `Tex_Place` (`0x00199B98`). It then draws the UTF-8 literal 奥十羽野駅 through `0x002469CC`. `Tex_Name` stays empty, so 「湖」 is only the badge glyph.
