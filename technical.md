...
| Piece | Role |
|-------|------|
| `timg/Eng_Patch.bclim` | Separate ETC1A4 strip (`Eng Patch v1.0.0-rc3-NENE` + newloveplus.loc.moe); **white glyphs + thick black outline** (readable on Main Menu white column). **NLPP-010 merge:** `v1.0.0-rc3-NENE` is a volunteer-build tag (`4450526`). Revert that commit before merging this branch into an RC feature branch. |
...
**Headline script-pack coverage:** **424 / 578 (73.4%)** files that *look* English in the CIA. That count **includes unreviewed Gemini Nene**. Legacy "28%" = community layer alone (~169 stems upstream / ~96 injected after Manaka/`p*` override).
...
**1822** SMS messages total (599 + 634 + 589). Included in fansite “scripts + SMS” rollup (**424 / 2400 = 17.7%** with Gemini Nene + current JP SMS).
...
*Last updated 2026-09-19 — Gemini Nene machine pass marked unreviewed; §21 TalkWindow table + voice/script cap (NLPP-025); UI PNG masters **1727** mapped / **562** chrome (`export_progress_metrics.py`); volunteer kit sizes in §20.2.*
