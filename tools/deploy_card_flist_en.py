...
def render_flist_label(w: int, h: int, text: str) -> Image.Image:
    """Match JP style: yellow bg, white outline, purple-blue fill."""
    bg = (255, 220, 0)
    ...
    for size in range(14, 7, -1):
        ...
        for ox, oy in (
            ...
            dr.text((x + ox, y + oy), text, font=f, fill=(255, 255, 255, 255))
        # fill (approx purple-blue)
        dr.text((x, y), text, font=f, fill=(120, 90, 220, 255))
        overlay = big.resize((w, h), Image.Resampling.BILINEAR)
...
def main() -> None:
    ...
    print(f"Card.arc dec={len(arc_elem.parsed())} slot={cmp_len}", flush=True)
