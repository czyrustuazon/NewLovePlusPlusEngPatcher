#!/usr/bin/env python3
"""EN Quit softkey やめる — ETC1A4 Com_btn_y01_b{,ON} @ NCommonIcon pkg 5238.

Zhoumaru master: assets/images/NCommonIcon.check/timg/Com_btn_y01_b{,ON}.png
Shared across menus (Save Export confirm, etc.). Last-writer for 5238 after
Back/Next/OK so those EN BCLIMs stay.

  python tools/deploy_softkey_quit_en.py
"""
...
OUT = ROOT / "out" / "softkey_quit_en"
FONT = UI_FONT
PKG = 5238
LABEL = "Quit"
TARGETS = [
    "timg/Com_btn_y01_b.bclim",
    "timg/Com_btn_y01_bON.bclim",
]

# If we have to rebuild from vanilla, keep the other 5238 last-writer labels.
KEEP_SOFTKEYS: list[tuple[str, str]] = [
...
def load_rgba(
    path: str, text: str, w: int, h: int, *, style: dict | None
) -> Image.Image:
    stem = Path(path).stem
    if style is None:
        master = find_ui_png(("NCommonIcon.check",), stem, (w, h))
        if master is not None:
            return _punch_black(Image.open(master).convert("RGBA"))
    on = "ON" in path
    return render_btn(w, h, text, on=on, **(style or STYLES[0]))
...
def _replace(
    ...
    *,
    ...
    rgba = load_rgba(path, text, w, h, style=style)
    png = tmp / "t.png"
    orig = tmp / "o.bclim"
    rgba.save(png)
    ...
    src = "master" if style is None else f"render {style}"
    print(f"OK {path} -> {text!r} ({src})", flush=True)


def _patch(arc_bytes: bytes, tmp: Path, *, style: dict | None, keep: bool) -> bytes:
    darc = DarcArchive(bytearray(arc_bytes))
    for path in TARGETS:
        _replace(darc, tmp, path, LABEL, style=style)
    if keep:
        for path, text in KEEP_SOFTKEYS:
            _replace(darc, tmp, path, text, style=style)
    return bytes(darc.data)


def _fit_slot(cand: bytes, cmp_len: int) -> tuple[bytes, bytes] | None:
    cand = _force_zero_gaps(cand)
    z = len(zopfli_zlib.compress(cand))
    print(f"  zopfli body={z} slot={cmp_len}", flush=True)
...
def main() -> int:
    ...
    # Live first so Back/Next/OK EN in the same ARC is kept.
    bases: list[tuple[str, Path, bool]] = [("live", MOD_IMG, False)]
    if VANILLA.is_file() and VANILLA.resolve() != MOD_IMG.resolve():
        bases.append(("vanilla", VANILLA, True))
    ...
    trials: list[dict | None] = [None, *STYLES]
    for base_name, base_path, keep in bases:
        ...
        for style in trials:
            cand = _patch(virgin, tmp, style=style, keep=keep)
            hit = _fit_slot(cand, cmp_len)
            if hit is None:
                continue
            tuned, slot = hit
            print(f"  exact-zlib hit ({base_name}, style={style})", flush=True)
            break
        if tuned is not None:
            break

    if tuned is None or slot is None or src_pkg is None or pkg is None:
        raise SystemExit(f"Quit softkey could not fit slot {cmp_len}")
    ...
    print("deployed Quit softkey ->", MOD_IMG, flush=True)
