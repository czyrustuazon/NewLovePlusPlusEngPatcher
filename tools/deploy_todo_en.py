#!/usr/bin/env python3
"""EN To-Do submenu: header (5575) + List/History buttons (Quest 5253)."""
...
OUT = ROOT / "out" / "todo_en"
FONT = Path(r"C:\Windows\Fonts\YuGothR.ttc")
INK = (70, 110, 160)

# Header variants in MyroomHeader (ETC1A4)
HEADER_PKG = 5575
HEADER_LABELS = [
    ("timg/mydata_toptex_01.bclim", "To-Do List"),  # ToDo 一覧
    ("timg/mydata_toptex_02.bclim", "To-Do List"),  # ToDoリスト (also used as title)
    ("timg/mydata_toptex_03.bclim", "History"),  # ToDo履歴
    ("timg/mydata_toptex_04.bclim", "Status"),  # ステータス
    ("timg/mydata_toptex_05.bclim", "Item List"),  # アイテム一覧
    ("timg/mydata_toptex_06.bclim", "My Camera"),  # マイカメラ
]

# Quest top select buttons + History points row (RGBA4444)
QUEST_PKG = 5253
QUEST_LABELS = [
    ("timg/Que_Txt01b.bclim", "To-Do List"),  # ToDoリスト
    ("timg/Que_Txt01d.bclim", "History"),  # ToDo履歴
    ("timg/Que_Txt01c.bclim", "Earned ToDo Points:"),  # 獲得ToDoポイント:
]
...
def patch_etc1_labels(
    ...
    for hard, max_size in trials:
        ...
        print(f"  trial hard={hard} max_size={max_size}: zopfli={z}", flush=True)
    ...
    if best[0] > cmp_len:
        raise SystemExit(f"no trial fits slot {cmp_len}")
    return best[1]


def patch_rgba_labels(vanilla_arc: bytes, labels: list[tuple[str, str]], tmp: Path) -> bytes:
    darc = DarcArchive(bytearray(vanilla_arc))
    for path, en in labels:
        entry = darc.find(path) or darc.find(Path(path).name)
        if entry is None:
        ...
        # Points row is left-aligned beside the numeric value.
        align_left = path.endswith("Que_Txt01c.bclim")
        # render_label is center-only here; bake left bias via wider canvas crop
        rgba = render_label(w, h, en, hard=False)
        if align_left:
            arr = np.array(rgba)
...
def main() -> None:
    ...
    img.parse(False)
    res = img.entries[HEADER_PKG]
    tmp_pkg = OUT / "img_data"
    tmp_pkg.mkdir(parents=True, exist_ok=True)
    src = tmp_pkg / f"{HEADER_PKG:04d}"
    ...
    cmp = arc.fw.len()
    print(f"pkg {HEADER_PKG} headers slot={cmp}", flush=True)
    tmp = OUT / "_fit_hdr"
    tmp.mkdir(parents=True, exist_ok=True)
    patched = patch_etc1_labels(arc.parsed(), HEADER_LABELS, tmp, cmp)
    splice_pkg(HEADER_PKG, MOD_IMG, patched, cmp)

    # Quest buttons from vanilla
    vraw = VANILLA.read_bytes()
    vimg = ImgBin(str(VANILLA))
    vimg.parse(False)
    res = vimg.entries[QUEST_PKG]
    src = tmp_pkg / f"{QUEST_PKG:04d}"
    src.write_bytes(vraw[res.fw.base_offset : res.fw.base_offset + res.fw.len()])
    pkg = Package(FileWindow(str(src)), 0)
    pkg.parse(False)
    arc = next(e for e in pkg.entries if isinstance(e, ARC))
    cmp = arc.fw.len()
    print(f"pkg {QUEST_PKG} Quest buttons slot={cmp}", flush=True)
    tmp = OUT / "_fit_quest"
    tmp.mkdir(parents=True, exist_ok=True)
    patched = patch_rgba_labels(arc.parsed(), QUEST_LABELS, tmp)
    splice_pkg(QUEST_PKG, VANILLA, patched, cmp)

    print("deployed To-Do EN ->", MOD_IMG, flush=True)
