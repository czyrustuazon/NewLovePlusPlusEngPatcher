#!/usr/bin/env python3
"""EN Gallery Common labels — pkg 5153.

Girl-select list chrome:
  Gallery_txt01 — Preview (ETC1A4)
  Gallery_txt02 — Slideshow (ETC1A4)
  Gallery_txt03 — All (ETC1A4)
  Gallery_txt04/05/06 — heroine names on save/card rows (RGBA4444)
  Gal_girl_select{M,N,R} — Dream Gallery girl buttons (RGBA4444)
"""
...
OUT = ROOT / "out" / "gallery_common_en"
FONT = UI_FONT
PKG = 5153

LABELS = [
    ("timg/Gallery_txt01.bclim", "Preview"),
    ("timg/Gallery_txt02.bclim", "Slideshow"),
    ("timg/Gallery_txt03.bclim", "All"),
    # Girl-select / card list name plates (120×18)
    ("timg/Gallery_txt04.bclim", "Takane Manaka"),
    ("timg/Gallery_txt05.bclim", "Kobayakawa Rinko"),
    ("timg/Gallery_txt06.bclim", "Anegasaki Nene"),
    # Dream Gallery girl buttons (108×16; M/N/R = Manaka/Nene/Rinko)
    ("timg/Gal_girl_selectM.bclim", "Takane Manaka"),
    ("timg/Gal_girl_selectR.bclim", "Kobayakawa Rinko"),
    ("timg/Gal_girl_selectN.bclim", "Anegasaki Nene"),
]
...
def render_label(w: int, h: int, text: str, fmt: int) -> Image.Image:
    """ETC1A4: white alpha strip. RGBA4444: dark-gray glyphs (name plates)."""
    if fmt == 0xB:
        ...
        raise RuntimeError(f"cannot fit label {text!r} into {w}x{h}")

    # RGBA4444 name plates — match vanilla dark-gray-on-clear style.
    im = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    dr = ImageDraw.Draw(im)
    for size in range(min(15, h + 1), 6, -1):
        f = font(size)
        b = dr.textbbox((0, 0), text, font=f)
        tw, th = b[2] - b[0], b[3] - b[1]
        if tw > w - 2:
        ...
        x = (w - tw) // 2 - b[0]
        y = (h - th) // 2 - b[1]
        dr.text((x, y), text, font=f, fill=(51, 51, 51, 220))
        return im
    raise RuntimeError(f"cannot fit name {text!r} into {w}x{h}")
...
def main() -> None:
    ...
    print(
        f"\n=== PKG {PKG} Gallery_Common labels dec={len(arc_elem.parsed())} "
        f"slot={cmp_len} ===",
        flush=True,
    )
    ...
    for path, en in LABELS:
        ...
        if fmt not in (0xB, 8):
            raise SystemExit(f"{path} fmt {fmt:#x} unsupported")
        png = tmp / f"{Path(path).stem}.png"
        orig = tmp / f"{Path(path).stem}.bclim"
        orig.write_bytes(raw)
        rgba = render_label(w, h, en, fmt)
        rgba.save(png)
        new = png_to_bclim_same_size(png, orig)
        rgba.save(OUT / f"{Path(path).stem}_en.png")
        darc.replace_same_size(entry, new)
        print(f"  OK {path} -> {en!r} ({w}x{h} fmt={fmt})", flush=True)
    ...
    print("\ndeployed Gallery_Common EN pkg", PKG, "->", MOD_IMG, flush=True)
    print("Rollback:", bak, flush=True)
    print(
        "Note: installed CIA may still have vanilla 5153 — rebuild CIA or "
        "confirm LayeredFS overrides after a full Azahar quit.",
        flush=True,
    )
