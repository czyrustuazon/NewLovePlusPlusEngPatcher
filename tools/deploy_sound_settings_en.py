#!/usr/bin/env python3
"""EN Sound Settings chrome in Option.arc pkg 5247."""
...
OUT = ROOT / "out" / "sound_set_en"
FONT = Path(r"C:\Windows\Fonts\YuGothR.ttc")
PKG = 5247
INK = (51, 51, 51)

# Labels on the Sound Settings panel (RGBA4444).
# NOTE: Opt_HelpBtn_* are Display Settings toggles (毎回/一度だけ), NOT Defaults.
# Prefer tools/deploy_display_settings_en.py which patches both panels correctly.
LABELS = [
...
def render_button(w: int, h: int, text: str) -> Image.Image:
    """White pill with grey border + centered dark text (keeps HelpBtn look)."""
    im = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    dr = ImageDraw.Draw(im)
    # Fill / border
    dr.rounded_rectangle((0, 0, w - 1, h - 1), radius=h // 2 - 1, fill=(255, 255, 255, 255))
    dr.rounded_rectangle(
        (0, 0, w - 1, h - 1), radius=h // 2 - 1, outline=(180, 180, 180, 255), width=1
    )
...
def main() -> None:
    ...
    print("deployed Sound Settings EN ->", MOD_IMG, flush=True)
