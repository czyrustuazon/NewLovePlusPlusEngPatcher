#!/usr/bin/env python3
"""EN boot CESA warning + companion blurb — img.bin package 90.

CESA_240X400.texi is the portrait warning; CESA_400X240.texi is the companion
slot (vanilla 16x16 stub — the blank screen beside CESA). Same 240x400
orientation as the warning. Opt-in during PNG pack (wrong zlib can white-boot).
Gold path splices a same-size PACK rebuild into release/bake_img.bin.
"""
...
def _refresh_pngs() -> None:
    """Rebuild CESA + companion masters from NLPPPATCH Heisei Gothic."""
    try:
        im = render_cesa_en()
...
def main() -> int:
    ...
    for dest in iter_deploy_targets(MOD_IMG):
        ...
        patch_img_bin_with_companion(
            dest, CESA_PNG, COMPANION_PNG, dest, work=work / dest.stem
        )
