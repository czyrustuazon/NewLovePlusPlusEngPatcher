...
### Facts you must use (2026-09-16)

**Dialogue / SMS / TRB:**

| Bucket | EN / total | % |
|--------|------------|---|
| Dialogue scripts (`script/*.dbin2`) | 424 / 578 | 73.4% |
| Manaka `t*` | 175 / 175 | 100% |
| Common `p*` | 55 / 55 | 100% |
| Rinko `k*` | 48 / 174 | 27.6% (NLPPATCH layer) |
| Nene `a*` | 146 / 174 | 83.9% — **145 files are a Gemini machine pass, unreviewed** (teal). ~28 `a*` still JP. |
| SMS (`maildic_*`, pkg 92) | 0 / 1822 | 0% (JP in gold bake) |
| Scripts + SMS rollup | 424 / 2400 | 17.7% |
| Main TRB STRI | 24872 / 25346 | 98.1% (machine pass, unreviewed) |

Do **not** treat 73.4% or 146 / 174 Nene as a finished route. Gemini stems use `bar: teal` / `caveat: "machine pass, unreviewed"` until listed in `assets/gemini_heroines/proofread.json`. NLPPATCH “28%” = 169 / 578 dialogue files (community layer alone).
...
| Homepage (`ProgressBar.astro`) | Comes from `progress.json`. Nene / total script bars must stay **teal** while `english_unreviewed` > 0. |
...
Same as the current Progress page: numbers are regenerated from the patcher, dialogue+SMS are the bottleneck, TRB 98% is a machine pass. **Nene’s English-looking files are also a machine pass** until proofread. Do not claim menus are “done” because TRB is high. Do not claim every in-game texture is English because 1727 masters exist.
